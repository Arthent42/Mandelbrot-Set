...
public class exapple extends Frame {
	...
	static public int iteration = 500;
	static public int clr = 0;
	static public int zoom = 1;
	static public int enter = 0;
	static public double dX = -3.0;
	static public double dXz = 3.0;
	static public double dY = -3.0;
	static public double dYz = 3.0;
	...
	public void paint(Graphics g) {
		while (true) {
			zoom = 1;
			s = (Math.abs(dX/zoom)+dXz/zoom)/600;
			int zeichne_x = 0;
			int zeichne_y = 0;
			int farbe = 0;
			
			for (double q = dY/zoom; q<=dYz/zoom;q=q+s) { // y
				zeichne_y++;
				zeichne_x=0;
				for (double p = dX/zoom; p<=dXz/zoom;p=p+s) { // x
					clr = 0;
					zeichne_x++;
